...
void main() {
  testWidgets('dragging across the canvas registers a stroke, and undo removes it', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(body: Center(child: DoodleCanvas(state: state))),
      ),
    );

    expect(state.strokes, isEmpty);

    final gesture = await tester.startGesture(tester.getCenter(find.byType(DoodleCanvas)));
  ...
  testWidgets('a plain tap (no drag) still registers as a single-point stroke', (tester) async {
    final state = DoodleCanvasState(color: Colors.black, brushWidth: 4);
    addTearDown(state.dispose);
